...
class Recipe(Dict):
    def __init__(
        ...
        super().__init__(
            id=id, title=title, ingredients=ingredients, steps=steps, likes=likes
        )
...
def lambda_handler(event, context):
    try:
        ...
        for recipe in recipes:
            ingredients = [
                Ingredient(ing["id"], ing["description"])
                for ing in recipe["ingredients"]
            ]
            steps = [Step(step["id"], step["description"]) for step in recipe["steps"]]
            recipes_list.append(
                Recipe(
                    recipe["id"], recipe["title"], ingredients, steps, recipe["likes"]
                )
            )
